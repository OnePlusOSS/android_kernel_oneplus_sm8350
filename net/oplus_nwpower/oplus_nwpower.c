...
static void __oplus_match_qrtr_wakeup(int src_node, int src_port, int dst_port,
				...
				u64 qrtr[][4], u64 wakeup[OPLUS_NW_WAKEUP_SUM], bool prt) {
	int i;
	int repeat[4] = {0};
	int repeat_index = 0;
	...
	if (atomic_read(&ipa_wakeup_hook_boot) == 1) {
		if (atomic_read(&tcp_is_input) == 0) {
			...
			if (((now_ts.tv_sec * 1000 + now_ts.tv_nsec / 1000000)
				- (tcp_last_transmission_stamp.tv_sec * 1000 + tcp_last_transmission_stamp.tv_nsec / 1000000))
...
static void print_ipa_wakeup(bool unsl,
			struct tcp_hook_struct *ptcp_in, struct tcp_hook_struct *ptcp_out,
