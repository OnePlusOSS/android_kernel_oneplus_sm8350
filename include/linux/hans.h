...
{
	return  (((task->group_leader->jobctl & JOBCTL_TRAP_FREEZE) == JOBCTL_TRAP_FREEZE)
		|| cgroup_task_frozen(task->group_leader) || freezing(task->group_leader) || frozen(task->group_leader));
