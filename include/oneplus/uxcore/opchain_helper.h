...
extern void opc_task_switch(bool enqueue, int cpu,
	struct task_struct *p, u64 clock);
extern int opc_get_claim_on_cpu(int cpu);
extern unsigned int opc_get_claims(void);
extern int opc_select_path(struct task_struct *cur,
	struct task_struct *t, int prev_cpu);
extern unsigned long opc_cpu_util(unsigned long util, int cpu,
	struct task_struct *t, int op_path);
...
static inline void opc_binder_pass(size_t data_size,
	uint32_t *data, int send) {}
static inline bool is_opc_task(struct task_struct *t, int type) { return 0; }
static inline void opc_task_switch(bool enqueue, int cpu,
	struct task_struct *p, u64 clock) {}
