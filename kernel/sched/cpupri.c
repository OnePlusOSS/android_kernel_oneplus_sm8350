 ...
 */
#if defined(OPLUS_FEATURE_SCHED_ASSIST) && defined(CONFIG_OPLUS_FEATURE_SCHED_ASSIST)
extern bool slide_task_misfit(struct task_struct *p, int cpu);
...
retry_vendor:
	for (idx = 0; idx < task_pri; idx++) {
		...
		if (drop_uxtasks && slide_task_misfit(p, task_cpu(p)) && (sched_assist_scene(SA_SLIDE)|| sched_assist_scene(SA_INPUT)))
