...
static int oplus_subsystem_schedule_kevent_fb_work(fb_tag tag_id,
		const char *event_id, unsigned char *str)
{
...
static void oplus_kevent_fb_upload_work(struct work_struct *work)
{
	...
	bsp_fb_work_struct *bsp_kevent = container_of(dwork, struct bsp_fb_work_struct,
					 dwork);

	oplus_subsystem_schedule_kevent_fb_work(bsp_kevent->tag_id,
						bsp_kevent->event_id, bsp_kevent->reason);
}
...
int oplus_kevent_fb_str(fb_tag tag_id, const char *event_id, unsigned char *str)
{
	...
	strlcpy((char *)&bsp_fb_dwork->event_id, event_id,
		sizeof(bsp_fb_dwork->event_id));
