...
static void update_procmem_entry_locked(int index, struct process_mem val,
					struct mem_mmap_data *buf)
{
...
static void update_meminfo_locked(int index, unsigned long value,
				  struct mem_mmap_data *buf)
{
...
static void midas_update_meminfo(struct midas_priv_info *info)
{
	...
	update_meminfo_locked(MI_MEMFREE,
			      (unsigned long)(i.freeram << (PAGE_SHIFT - 10)), info->mmap_addr);
	update_meminfo_locked(MI_MEMAVAIL, available << (PAGE_SHIFT - 10),
			      info->mmap_addr);
