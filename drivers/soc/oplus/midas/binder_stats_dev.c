...
static int user_enable_binder_stats(struct binder_stats_user_context *context_ptr,
					int enable)
{
	unsigned long flags;
	struct binder_stats_item_ref *ref_hash_node;
...
static int user_binder_stats_cfg_set_max_item_cnt(struct binder_stats_user_context *context_ptr,
							int max_item_cnt)
{
	if (1 == context_ptr->enable) {
		BINDER_STATS_LOGE("can not set config when function enabled already!\n");
