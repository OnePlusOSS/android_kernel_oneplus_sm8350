...
/*
 * Silver cluster, param@0: affect_freq1, param@1: \
 * affect_thres1, param@2: affect_freq2, param@3: \
 * affect_thres2, param@4 need mask freq.
 */
static int cluster0_effiency[MAX_CLUSTER_PARAMETERS] = { 0, 0, 0, 0, 0};
module_param_array(cluster0_effiency, int, NULL, 0664);

/*
 * Gold cluster, param@0: affect_freq1, param@1: \
 * affect_thres1, param@2: affect_freq2, param@3: \
 * affect_thres2, param@4 need mask freq.
 */
...
/*
 * Gold_plus cluster, param@0: affect_freq1, \
 * param@1: affect_thres1, param@2: affect_freq2, \
 * param@3: affect_thres2, param@4 need mask freq.
 */
...
static int get_cluster_num(struct cpufreq_policy *policy)
{
	...
	pr_debug("cluster_num: %d platform_soc_id: %d opp_number: %d.\n", cluster_num, platform_soc_id, *(opp_number + cluster_num));
...
static bool was_diff_powerdomain(struct cpufreq_policy *policy, unsigned int freq)
{
	...
	if (index == index_pre) {
		pr_debug("index: %d index_pre: %d\n", index, index_pre);
		return false;
	}

	cluster_pd = get_cluster_pd(policy);
	if (cluster_pd != NULL) {
		pr_debug("freq: %d cluster_pd[index]: %d cluster_pd[index_pre]: %d\n", freq,
				cluster_pd[index], cluster_pd[index_pre]);
...
{
	unsigned int target_freq, index, affect_thres;
	int cluster_id;

	index = cpufreq_frequency_table_target(policy, freq - 1, CPUFREQ_RELATION_H);
	target_freq = policy->freq_table[index].frequency;

	if ((loadadj_freq > freq) || (loadadj_freq < target_freq))
		return freq;
	...
	if (abs(loadadj_freq - target_freq) < affect_thres)
		return target_freq;
...
static int cpufreq_pd_init(void)
{
	...
	pr_info("platform_soc_id = %d prop_str = %s\n", platform_soc_id, prop_str);
...
static int frequence_opp_init(struct cpufreq_policy *policy)
{
	...
	pr_info("*(opp_number + cluster_id %d): %d opp_number: %d\n", cluster_id,
		*(opp_number + cluster_id), opp_num);
