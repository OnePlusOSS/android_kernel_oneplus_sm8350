...
static void cb_core_boost(u64 time)
{
	...
	if (debug)
		pr_info("core_ctl_boost: %d %d %llu\n", need_boost,
			last_core_boost, last_core_boost_ts);
...
void cb_update(struct cpufreq_policy *pol, u64 time)
{
	...
	/*
	 * check current min_freq and limit target freq,
	 * if min_freq large than limited, reset acc
	 **/
	...
	if (debug)
		pr_info("cpu %d update: ts now %llu last %llu last_update %llu delta %llu update_d %llu \
			cur %u acc %llu, cur_level %d min_over_target_freq %d isolated %d last_core_boost %d\n",
			cpu,
			NSEC_TO_MSEC(time),
