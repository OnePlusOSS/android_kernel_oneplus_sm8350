...
void do_chrdet_int_task(void)
{
	...
	{
		battery_xlog_printk(BAT_LOG_CRTI, "[do_chrdet_int_task] warp || gauge || charger not ready, will do after bettery init.\n");
		return;
	}

	if(oplus_warp_get_fastchg_started() == true && oplus_warp_get_adapter_update_status() != ADAPTER_FW_NEED_UPDATE){
...
static long adc_cali_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case Get_FakeOff_Param: /* For Factory Mode*/
		...
		fakeoff_out_data[3] = oplus_warp_get_fastchg_started();
