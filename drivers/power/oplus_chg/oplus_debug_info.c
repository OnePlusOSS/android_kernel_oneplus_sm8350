...
struct warp_charge_strategy {
	...
	struct input_current_setting
		input_current[OPLUS_CHG_WARP_INPUT_CURRENT_MAX_CNT];
};
...
struct warp_charge_strategy *
	g_warp_charge_strategy_p[OPLUS_FASTCHG_OUTPUT_POWER_MAX] = { NULL, NULL,
				 NULL,
				 NULL
};
...
static struct oplus_chg_debug_notify_policy oplus_chg_debug_notify_policy[] = {
	...
	[OPLUS_NOTIFY_BATT_SOC_CAPCITY_JUMP] = {
		OPLUS_CHG_NOTIFY_REPEAT, 1, 1,
		-1
	},
	[OPLUS_NOTIFY_BATT_UI_SOC_CAPCITY_JUMP] = {
		OPLUS_CHG_NOTIFY_REPEAT, 1,
		1, -1
	},
	[OPLUS_NOTIFY_BATT_UI_TO_SOC_CAPCITY_JUMP] = {
		OPLUS_CHG_NOTIFY_REPEAT,
	...
	[OPLUS_NOTIFY_CHG_SLOW_BATT_NON_AUTH] = {
		OPLUS_CHG_NOTIFY_ONCE, 6, 6,
		-1
	},
	[OPLUS_NOTIFY_CHG_SLOW_NON_OPLUS_CHARGER] = {
		OPLUS_CHG_NOTIFY_ONCE, 6,
		6, -1
	},
	...
	[OPLUS_NOTIFY_CHG_SLOW_CHECK_TIME] = {
		OPLUS_CHG_NOTIFY_REPEAT, 720,
		1440, -1
	},

	[OPLUS_NOTIFY_CHG_BATT_FULL_NON_100_CAP] = {
		OPLUS_CHG_NOTIFY_ONCE, 3,
		3, -1
	},
	[OPLUS_NOTIFY_CHG_BATT_FULL_AND_100_CAP] = {
		OPLUS_CHG_NOTIFY_ONCE, 3,
...
static void oplus_chg_send_info_dwork(struct work_struct *work)
{
	...
	if ((ret > 0) && (oplus_chg_debug_info.retry_cnt > 0)) {
		...
		oplus_chg_debug_mask_notify_flag(
			OPLUS_NOTIFY_CHG_SLOW_CHECK_TIME + 1,
			OPLUS_NOTIFY_CHG_FULL);

		oplus_chg_debug_mask_notify_flag(OPLUS_NOTIFY_CHG_FULL + 1,
						 OPLUS_NOTIFY_BATT_AGING);
...
static void oplus_chg_print_debug_info(struct oplus_chg_chip *chip)
{
...
#endif
	{
		...
		ret += snprintf(&oplus_chg_debug_msg[ret],
				OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$flag@@0x%x",
				oplus_chg_debug_info.notify_flag);

		ret += snprintf(&oplus_chg_debug_msg[ret],
				OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$pre_soc@@%d",
		...
		ret += snprintf(&oplus_chg_debug_msg[ret],
				OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$soc_load@@%d",
				chip->soc_load);
		ret += snprintf(&oplus_chg_debug_msg[ret],
		...
		ret += snprintf(
			       ...
			       tm.tm_min, tm.tm_sec, chip->charger_exist,
		...
		if (warp_chip) {
			ret += snprintf(&oplus_chg_debug_msg[ret],
					OPLUS_CHG_DEBUG_MSG_LEN - ret,
					"WARP[ %d / %d / %d / %d / %d / %d]",
					warp_chip->fastchg_allow,
					warp_chip->fastchg_started,
					warp_chip->fastchg_dummy_started,
					warp_chip->fastchg_to_normal,
					warp_chip->fastchg_to_warm,
					warp_chip->btb_temp_over);
		}
...
{
	int i;
	int arr_size = sizeof(warp_charger_type_list) /
		       sizeof(warp_charger_type_list[0]);

	for (i = 0; i < arr_size; i++) {
		if (oplus_warp_get_fast_chg_type() ==
				warp_charger_type_list[i].chg_type)
			break;
	}
...
static int
oplus_chg_get_warp_adapter_is_low_input_power(struct oplus_chg_chip *chip)
{
	...
	adapter_input_power =
		warp_charger_type_list[warp_adapter_type_index].vol *
		warp_charger_type_list[warp_adapter_type_index].cur;

	if (adapter_input_power < (oplus_chg_debug_info.warp_max_input_volt *
				   oplus_chg_debug_info.warp_max_input_current))
		return 1;
...
static int oplus_chg_get_warp_chg_max_input_power(struct oplus_chg_chip *chip)
{
	...
	return (oplus_chg_debug_info.warp_max_input_volt *
		oplus_chg_debug_info.warp_max_input_current * 8 / 10);
}
...
static int oplus_chg_get_warp_input_power(struct oplus_chg_chip *chip)
{
	...
	vol = oplus_chg_debug_info.warp_max_input_volt <
	      warp_charger_type_list[i].vol ?
	      oplus_chg_debug_info.warp_max_input_volt :
	      warp_charger_type_list[i].vol;
	cur = oplus_chg_debug_info.warp_max_input_current <
	      warp_charger_type_list[i].cur ?
	      oplus_chg_debug_info.warp_max_input_current :
	      warp_charger_type_list[i].cur;

	if (warp_chip->warp_chg_current_now > 0)
		cur = cur < warp_chip->warp_chg_current_now ?
		      cur :
		      warp_chip->warp_chg_current_now;
	...
	if (oplus_chg_debug_info.warp_charge_strategy) {
		...
		mcu_chg_current =
			oplus_chg_debug_info.warp_charge_strategy
			->input_current
			[oplus_chg_debug_info
			 .warp_charge_input_current_index]
			.cur;
...
static int oplus_chg_warp_charge_strategy_init(struct oplus_chg_chip *chip)
{
	...
	if (oplus_chg_debug_info.warp_charge_strategy != NULL) {
		for (i = oplus_chg_debug_info.warp_charge_input_current_index;
				i < OPLUS_CHG_WARP_INPUT_CURRENT_MAX_CNT; i++) {
			if (chip->batt_volt <=
					oplus_chg_debug_info.warp_charge_strategy
					->input_current[i]
					.vol) {
				if (i ==
						...
						.warp_charge_input_current_index)
					oplus_chg_debug_info
					.warp_charge_cur_state_chg_time +=
						5;
				...
				if (oplus_chg_debug_info.warp_charge_strategy
					...
					if (oplus_chg_debug_info
							.warp_charge_cur_state_chg_time >=
							oplus_chg_debug_info
							.warp_charge_strategy
							->input_current[i]
							.time) {
						if (i <
								(OPLUS_CHG_WARP_INPUT_CURRENT_MAX_CNT -
								 1))
							i++;
					}
				}
	...
	if (!oplus_chg_chg_is_normal_path(chip)) {
		if (oplus_warp_get_fastchg_started()) {
			...
			output_power_type =
				warp_charger_type_list[index].output_power_type;
		}
	}
	...
	if (output_power_type >= 0) {
		...
		warp_charge_strategy_p =
			g_warp_charge_strategy_p[output_power_type];

		if (warp_charge_strategy_p) {
			for (i = 0;
					warp_charge_strategy_p[i].capacity_range.low != -1;
					i++) {
				if ((warp_charge_strategy_p[i]
						.capacity_range.low < start_soc) &&
						(warp_charge_strategy_p[i]
						 .capacity_range.high >=
						 start_soc) &&
	...
	if (oplus_chg_debug_info.warp_charge_strategy) {
		for (i = oplus_chg_debug_info.warp_charge_input_current_index;
			...
			if (chip->batt_volt <=
					...
					.vol) {
				oplus_chg_debug_info
				.warp_charge_input_current_index = i;
				oplus_chg_debug_info
				.warp_charge_cur_state_chg_time = 5;
				break;
			}
		}
	}
...
static int
oplus_chg_debug_fastchg_monitor_condition(struct oplus_chg_chip *chip)
{
	if (oplus_chg_debug_info.fastchg_stop_cnt <
			OPLUS_CHG_DEBUG_FASTCHG_STOP_CNT) {
...
static int oplus_chg_normal_chg_input_power_check(struct oplus_chg_chip *chip)
{
	...
	if ((chip->batt_volt * 1000) < float_voltage) {
		if (((input_current_settled * input_voltage_settled / 1000) -
				(chip->ibus * chip->charger_volt)) >
...
static int oplus_chg_fastchg_chg_input_power_check(struct oplus_chg_chip *chip)
{
	...
	if (((input_power + charge_power) > INPUT_POWER_DECREASE) &&
			(!chip->led_on) &&
	...
	if (warp_chip) {
		if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
			if (warp_chip->fastchg_to_warm) {
				...
				if ((chip->soc < warp_chip->warp_high_soc) &&
						(chip->soc > warp_chip->warp_low_soc)) {
					if (chip->temperature >=
							warp_chip->warp_high_temp) {
						oplus_chg_set_chg_flag(
							OPLUS_NOTIFY_CHG_SLOW_BATT_WARM_TEMP);
					}

					if (chip->temperature <=
							warp_chip->warp_low_temp) {
						oplus_chg_set_chg_flag(
							OPLUS_NOTIFY_CHG_SLOW_BATT_COLD_TEMP);
					}
				}
	...
	if (chip->charger_exist) {
		if (!oplus_chg_debug_info.total_time) {
			...
			oplus_chg_debug_info.total_time =
				5 + ts.tv_sec - charge_start_ts.tv_sec;
		}
	...
	if (chip->warp_project) {
		{
			if ((oplus_chg_fastchg_is_allowed(chip) == 1) &&
				...
				if (oplus_chg_get_warp_adapter_is_low_input_power(
							chip)) {
					oplus_chg_set_chg_flag(
						OPLUS_NOTIFY_CHG_SLOW_WARP_ADAPTER_NON_MAX_POWER);
				}
...
static int oplus_chg_chg_full_check(struct oplus_chg_chip *chip)
{
	...
	if ((status == POWER_SUPPLY_STATUS_FULL) && (capacity != 100) &&
		...
		if (!(oplus_chg_debug_info.chg_full_notified_flag & (1 << 1))) {
			...
			if (oplus_chg_debug_notify_flag_is_set(
						OPLUS_NOTIFY_CHG_BATT_FULL_AND_100_CAP)) {
				oplus_chg_debug_info.chg_full_notified_flag |=
					(1 << 1);
				oplus_chg_set_notify_type(
					OPLUS_CHG_DEBUG_NOTIFY_TYPE_CHG_FULL);
			}
...
static int oplus_chg_chg_batt_aging_check(struct oplus_chg_chip *chip)
{
...
#endif
	{
		...
		if ((aging_cap_test > 0) || (ts.tv_sec - last_ts.tv_sec >
...
static int oplus_chg_chg_batt_capacity_jump_check(struct oplus_chg_chip *chip)
{
	if (oplus_chg_debug_info.cur_soc == OPLUS_CHG_BATT_INVALID_CAPACITY) {
		...
		} else if (oplus_chg_debug_info.cur_soc ==
				oplus_chg_debug_info.pre_soc) {
		...
		} else if (oplus_chg_debug_info.cur_ui_soc ==
				oplus_chg_debug_info.pre_ui_soc) {
			oplus_chg_unset_soc_notified_flag(
				OPLUS_NOTIFY_BATT_UI_SOC_CAPCITY_JUMP);
		}
...
static int oplus_chg_debug_info_parse_dt(void)
{
	...
	int size;
	u32 val[2];
	char *warp_adpater_name[] = { "warp_charge_strategy_20w",
	...
	ret = of_property_read_u32(warp_node,
				   "qcom,warp-max-input-volt-support",
				   &oplus_chg_debug_info.warp_max_input_volt);
	...
	ret = of_property_read_u32(
		      warp_node, "qcom,warp-max-input-current-support",
		      &oplus_chg_debug_info.warp_max_input_current);
	...
	for (i = 0; i < OPLUS_FASTCHG_OUTPUT_POWER_MAX; i++) {
		...
		g_warp_charge_strategy_p[i] = kzalloc(
						      (count + 1) * sizeof(struct warp_charge_strategy),
						      GFP_KERNEL);
		...
		for_each_child_of_node(node, child) {
			...
			if (!ret) {
				g_warp_charge_strategy_p[i][j].temp_range.low =
					val[0];
				g_warp_charge_strategy_p[i][j].temp_range.high =
					val[1];
			}
			...
			if (size >= 0) {
				for (k = 0;
						(k < (size / 3)) &&
						(k < OPLUS_CHG_WARP_INPUT_CURRENT_MAX_CNT);
						k++) {
...
static int oplus_chg_debug_info_reset(void)
{
	...
	for (i = OPLUS_NOTIFY_CHG_SLOW_NON_OPLUS_CHARGER;
			i <= OPLUS_NOTIFY_CHG_SLOW_CHECK_TIME; i++)
		oplus_chg_debug_info.chg_cnt[i] = 0;

#else
	memset(oplus_chg_debug_info.chg_cnt, 0x0,
	       sizeof(oplus_chg_debug_info.chg_cnt));
#endif
