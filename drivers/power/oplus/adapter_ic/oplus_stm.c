...
static bool warp_uart_read_addr_line_and_check(unsigned int addr) {
	...
	check_result = true;
	read_addr_line_err: warp_uart_irq_fiq_enable(true);
	if (addr_check_err) {
	...
	if (!check_result) {
		chg_err(
				" fw_check err, addr:0x%x, check_buf[%d]:0x%x != fw_data[%d]:0x%x\n",
...
static bool warp_adapter_update_handle(unsigned long tx_pin,
	...
	for (rx_last_line_count = 0; rx_last_line_count < 18;
			rx_last_line_count++) {
		rx_last_line[rx_last_line_count] = warp_uart_rx_byte(
				Read_Addr_Line_Cmd);
		if (chip->rx_timeout) {
			break;
		}
	...
	if ((rx_last_line[FW_EXIST_LOW] == 0x55
			&& rx_last_line[FW_EXIST_HIGH] == 0x34) || chip->rx_timeout) {
	...
	update_err: chip->rx_timeout = false;
	chip->adapter_update_ing = false;
	chg_err(" err\n");
	return false;
}
...
struct oplus_adapter_operations oplus_adapter_ops = { .adapter_update =
		warp_adapter_update_handle, };
