...
static int oplus_warp_set_current_temp_little_cold_range(struct oplus_warp_chip *chip, int vbat_temp_cur)
{
	...
	if (chip->warp_batt_over_high_temp != -EINVAL
		...
		chip->warp_temp_cur_range = FASTCHG_TEMP_RANGE_INIT;
		oplus_warp_reset_temp_range(chip);
		if (vbat_temp_cur >= chip->warp_little_cold_temp) {
			chip->warp_little_cold_temp -= WARP_TEMP_RANGE_THD;
		}
	}
...
static void oplus_warp_fastchg_func(struct work_struct *work)
{
	...
	if (data == WARP_NOTIFY_FAST_ABSENT ||
...
{
	if (oplus_warpphy_get_fastchg_start() == true) {
		return true;
	}
...
{
	if (oplus_warpphy_get_fastchg_ing() == true) {
		return true;
	}
...
{
	if (oplus_warpphy_get_fastchg_to_normal() == true) {
		return true;
	}
...
{
	if (oplus_warpphy_get_fastchg_to_warm() == true) {
		return true;
	}
...
{
	if (oplus_chg_get_warpphy_support() == true) {
		return true;
	}
...
{
	if (oplus_warpphy_get_fastchg_dummy_start() == true) {
		return true;
	}
...
void oplus_warp_reset_mcu(void)
{
	...
	if (!g_warp_chip) {
		...
		if ((mcu_hwid_type == OPLUS_WARP_ASIC_HWID_RK826
			...
			&& g_warp_chip->warp_reply_mcu_bits == 7) {
			usleep_range(100000, 100000);
			if (g_warp_chip->vops->update_temperature_soc != NULL)
				g_warp_chip->vops->update_temperature_soc();
		}
	}
}
...
int oplus_warp_get_fast_chg_type(void)
{
	...
	fast_chg_type = oplus_warpphy_get_fast_chg_type();
