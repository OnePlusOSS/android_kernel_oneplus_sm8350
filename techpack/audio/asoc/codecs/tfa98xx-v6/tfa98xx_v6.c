...
*/
extern int send_tfa_cal_apr(void *buf, int cmd_size, bool bRead, int mi2s_id);
extern int send_tfa_cal_in_band(void *buf, int cmd_size, int mi2s_id);
#else /*OPLUS_FEATURE_TFA98XX_VI_FEEDBACK*/
#define send_tfa_cal_apr(buf, cmd_size, bRead, mi2s_id) (0)
#define send_tfa_cal_in_band(buf, cmd_size, mi2s_id) (0)
...
static int tfa98xx_send_volume(uint8_t channel, uint8_t volume)
{
	...
	return send_tfa_cal_in_band(&cmd[0], sizeof(cmd), tfa_mi2s_id);
}
				     ...
				     loff_t *ppos)
{
	...
	mutex_lock(&tfa98xx->dsp_lock);
	if (tfa98xx->tfa->is_probus_device)
		ret = send_tfa_cal_apr(buffer, count, true, tfa98xx->tfa->mi2s_id);
	else
				     ...
				     size_t count, loff_t *ppos)
{
	...
	if (tfa98xx->tfa->is_probus_device) {
		...
		err = send_tfa_cal_apr(buffer, count, false, tfa98xx->tfa->mi2s_id);
		if (err) {
...
tfa98xx_write_dsp(struct tfa_device *tfa,  int num_bytes, const char *command_buffer)
{
	...
	err = send_tfa_cal_apr(buffer, num_bytes, false, tfa_mi2s_id);
	if (err) {
...
tfa98xx_read_dsp(struct tfa_device *tfa,  int num_bytes, unsigned char *result_buffer)
{
	...
	ret = send_tfa_cal_apr(result_buffer, num_bytes, true, tfa_mi2s_id);
...
enum Tfa98xx_Error tfa98xx_adsp_send_calib_values(void)
{
	...
	if (tfa98xx_device_count == bytes[0]) {
		...
		ret = send_tfa_cal_in_band(&bytes[1], sizeof(bytes) - 1, tfa_mi2s_id);
		#ifdef OPLUS_FEATURE_FADE_IN
		tfa98xx_dsp_cmd = 1;
		#endif
		msleep(10);
			     ...
			     const struct i2c_device_id *id)
{
	...
	tfa_mi2s_id = tfa98xx->tfa->mi2s_id;
