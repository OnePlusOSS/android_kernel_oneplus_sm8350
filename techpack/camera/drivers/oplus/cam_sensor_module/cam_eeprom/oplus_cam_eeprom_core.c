		      ...
		      uint32_t *data)
{
	...
	for (i = 0; i < retry; i++) {
		if (e_ctrl->change_cci && chip_version_old == FALSE) {
			rc = camera_io_dev_read(&(e_ctrl->io_master_info_ois), (uint32_t)addr,
						(uint32_t *)data,
			...
			rc = camera_io_dev_read(&(e_ctrl->io_master_info), (uint32_t)addr,
						(uint32_t *)data,
						CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_DWORD);
				 ...
				 struct cam_eeprom_memory_map_t    *emap, int j, uint8_t *memptr)
{
	...
	for (i = 0; i < ((emap[j].mem.valid_size / 4) / 32 + 1); i++) {
		...
		rc = EEPROM_FlashMultiRead(e_ctrl, USER_MAT, emap[j].mem.addr + i * 32, data,
					   read_size);
				...
				struct cam_write_eeprom_t *cam_write_eeprom)
{
	...
	for (i = 0; i < cam_write_eeprom->calibDataSize;) {
		...
		for (j = 0; (j < WRITE_DATA_MAX_LENGTH
				&& i < cam_write_eeprom->calibDataSize); j++) {
			i2c_reg_arrays[j].reg_addr = CMD_IO_WRDAT_ACCESS;
			i2c_reg_arrays[j].reg_data = (((cam_write_eeprom->calibData[i] << 24) &
						       0xFF000000)
			...
			CAM_ERR(CAM_EEPROM, "write regs:0x%0x = 0x%0x", i2c_reg_arrays[j].reg_addr,
				...
				struct cam_write_eeprom_t *cam_write_eeprom)
{
	...
	struct cam_sensor_i2c_reg_array    *i2c_reg_arrays
		= (struct cam_sensor_i2c_reg_array *)kmalloc((sizeof(struct
				cam_sensor_i2c_reg_array) * SEMCO_EEPROM_PACK_SIZE),
				GFP_KERNEL);
	...
	for (i = 0; i < m_eeprom_size;) {
		...
		i2c_reg_settings.data_type = CAMERA_SENSOR_I2C_TYPE_BYTE;
		i2c_reg_settings.reg_setting = i2c_reg_arrays;
		rc = camera_io_dev_write_continuous(&e_ctrl->io_master_info, &i2c_reg_settings,
						    1);
			       ...
			       struct cam_write_eeprom_t *cam_write_eeprom)
{
	...
	if (rc < 0)
		CAM_ERR(CAM_EEPROM,
			"EEPROM fm24pc256e write cc enable failed, fallback to operate gt24p128c2csli");
			     ...
			     struct cam_write_eeprom_t *cam_write_eeprom)
{
	...
	if (((cam_write_eeprom->cam_id == 0x01) || (cam_write_eeprom->cam_id == 0x02)
				     ...
				     void *arg)
{
	...
	if (strncmp(cam_write_eeprom.eepromName, "imx766_fm24pc256e_gt24p128c2csli",
			20) == 0)
		is_fm24pc256e = true;
	...
	else if (strncmp(cam_write_eeprom.eepromName, "imx766_gt24p128c2csli_m",
			 23) == 0)
		is_gt24p128m = true;

	else if (strncmp(cam_write_eeprom.eepromName, "imx766_gt24p128c2csli_w",
			 23) == 0)
		is_gt24p128w = true;
				     ...
				     void *arg)
{
	...
	if ((check_eeprom_data.cam_id == 0x01 || check_eeprom_data.cam_id == 0x02
		...
		CAM_DBG(CAM_EEPROM, "eepromData_checksum: %d",
