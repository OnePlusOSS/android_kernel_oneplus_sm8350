/***************************************************************
** File : slabtrace.c
** Description : enable the feature when debug slub memory leak
** Version : 1.0
** Date : 2020/09/21
******************************************************************/
#include "slabtrace.h"
				...
				const struct track *track)
{
	 ...
	 */
	if (t->count >= t->max &&
		!alloc_loc_track(t, 2 * t->max, GFP_ATOMIC))
		return 0;
...
static int oplus_memcfg_slabtrace_show(struct seq_file *m, void *p)
{
	...
	list_for_each_entry(s, &slab_caches, list) {
		 ...
		 */
		if (!strcmp(sort_cache_name, "kmalloc-")) {
			if (!strstr(s->name, "kmalloc-"))
				continue;
			else
				goto out;
		}
		if (strcmp(s->name, sort_cache_name))
			continue;
...
static ssize_t slabcfg_write(struct file *filp, const char *ubuf, size_t cnt, loff_t *data)
{
	...
	if (!strcmp(argv1, "reset")) {
		...
		strcpy(sort_cache_name, "kmalloc-");
		goto out;
	}
	/* print type addr */
