...
static bool _iris_check_fw_file_isZero(const struct firmware *ccf_fw, const uint8_t *name)
{
	int i = 0;
	...
	for (i = 0; i < ccf_fw->size/5; i++){
		if (ccf_fw->data[i])
			return false;
	}
	IRIS_LOGE("%s(%d), %s file is zero!!!",__func__, __LINE__, name);
	return true;
}
...
int iris_parse_lut_cmds(u8 flag)
{
	...
	if (flag == 0 || flag == 2) {
		...
		is_zero = _iris_check_fw_file_isZero(ccf1_fw,ccf1_name);
		if ((ret && flag == 0) || is_zero) {
	...
	if (flag == 0 || flag == 2) {
		...
		is_zero = _iris_check_fw_file_isZero(ccf2_fw,ccf2_name);
		if ((ret && flag == 0) || is_zero) {
			// Load golden firmware.
